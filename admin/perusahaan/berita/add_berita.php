...
$carikode = mysqli_query($koneksi,"SELECT id_berita FROM tb_berita order by id_berita desc");
$datakode = mysqli_fetch_array($carikode);
$kode = isset($datakode['id_berita']) ? $datakode['id_berita'] :'';
	...
	<form action="" method="post" enctype="multipart/form-data">
					<div class="box-body">
						<div class="form-group">
							...
							<input type="text" name="id_berita" id="id_berita" class="form-control" value="<?php echo $format; ?>"
							 readonly/>
						</div>
			...
			<div class="form-group row">
				...
				<div class="col-sm-8">
					<input type="text" class="form-control" id="judul" name="judul" placeholder="Nama" readonly>
				</div>
			</div>
		...
		<div class="card-footer">
			...
			<a href="?page=prs-data-berita" title="Kembali" class="btn btn-secondary">Batal</a>
		</div>
	</form>
</div>
...
<?php
    ...
    if (isset ($_POST['Simpan'])){
		...
		if(!empty($sumber)){
			$sql_simpan = "INSERT INTO tb_berita (id_berita,judul,penerbit,tgl,filee) VALUES (
			   '".$_POST['id_berita']."',
			  '".$_POST['judul']."',
			  '".$_POST['penerbit']."',
			  '".$tanggal."',
    ...
    if ($query_simpan){
      ...
      Swal.fire({title: 'Tambah Data Berhasil',text: '',icon: 'success',confirmButtonText: 'OK'
          ...
          if (result.value) {
              window.location = 'page.php?page=prs-data-berita';
          }
      })</script>";
      }else{
      echo "<script>
      Swal.fire({title: 'Tambah Data Gagal',text: '',icon: 'error',confirmButtonText: 'OK'
          ...
          if (result.value) {
              window.location = 'page.php?page=prs-add-berita';
          }
	  })</script>";
		}
		}elseif(empty($sumber)){
